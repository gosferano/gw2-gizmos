...
/// <summary>
/// Bridges notifications produced by *other* processes (e.g. the ingestion worker) into Herald.
/// Polls the <c>Notifications</c> table for new rows whose <c>Source</c> isn't Herald, fires a toast
/// for each, and publishes them to the in-app feed. Herald's own notifications are already toasted
...
public sealed class NotificationWatcher : BackgroundService
{
    ...
    private async Task PollAsync(CancellationToken stoppingToken)
    {
        ...
        {
            // Skip Herald's own rows (handled in-process); surface everything else.
