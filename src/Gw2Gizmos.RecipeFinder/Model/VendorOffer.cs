...
/// </summary>
public sealed record VendorCost(long Amount, string Currency, int? ItemId, string? IconUrl)
{
    /// <summary>The currency is a tradeable item — render its icon with the item-id <c>ItemImage</c>.</summary>
    public bool HasItemIcon => ItemId is > 0;

    /// <summary>The currency is an account currency with a resolved icon — render it with <c>CurrencyImage</c>.</summary>
    public bool HasCurrencyIcon => !HasItemIcon && !string.IsNullOrEmpty(IconUrl);
}
